...
net.mode = 'test';
batch = 64;
sampling = floor(length(params.train) / 5000);
...
for i = 1:num_batch
    ...
    images = bsxfun(@minus, single(images), net.meta.normalization.averageImage) ;
    net.eval({'data', images});
    if strcmp(net.layers(end).block.lossType,'CE')
        feats(:,(i-1)*batch+1:i*batch) = squeeze(gather(vl_nnsoftmax(net.vars(net.getVarIndex(tlayerName)).value))) ;
    else
        feats(:,(i-1)*batch+1:i*batch) = squeeze(gather(net.vars(net.getVarIndex(tlayerName)).value)) ;
    end
end
if num_batch*batch < length(scores_train)
    images = gpuArray(data(:,:,:,num_batch*batch+1:length(scores_train)));
    images = bsxfun(@minus, single(images), net.meta.normalization.averageImage) ;
    net.eval({'data', images}) ;
    if strcmp(net.layers(end).block.lossType,'CE')
        feats(:,num_batch*batch+1:length(scores_train)) = squeeze(gather(vl_nnsoftmax(net.vars(net.getVarIndex(tlayerName)).value)));
    else
        feats(:,num_batch*batch+1:length(scores_train)) = squeeze(gather(net.vars(net.getVarIndex(tlayerName)).value));
    end
end
